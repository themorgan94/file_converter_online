...
def convert_each(file_object, dest_format, options, config):
    ...
    command = ["clear"]
    if dest_format.lower() in ["jpg","jpeg"]:
        command = ["jpegoptim", "-m", "50", inputfile]
    elif dest_format.lower() in ["png"]:
        command = ["pngquant", "--force", "--quality=30-100","--strip","--skip-if-larger","--verbose", inputfile, "--output", outputfile]
    elif dest_format.lower() in ["svg"]:
        command = ["svgo", inputfile, "-o", outputfile]
